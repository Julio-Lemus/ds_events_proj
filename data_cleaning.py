...
# #google anlaytics
df2['ga4_yn'] = df['Job Description'].apply(lambda x: 1 if 'google analytics' in x.lower() or 'r-studio' in x.lower() else 0)
df2.ga4_yn.value_counts()

#semrush 
df2['semrush_yn'] = df['Job Description'].apply(lambda x: 1 if 'semrush' in x.lower() else 0)
df2.semrush_yn.value_counts()

#hubspot 
df2['hubspot_yn'] = df['Job Description'].apply(lambda x: 1 if 'hubspot' in x.lower() else 0)
df2.hubspot_yn.value_counts()
...
#adobe
df2['adobe_yn'] = df['Job Description'].apply(lambda x: 1 if 'adobe' in x.lower() else 0)
...
#Power point
df2['powerpoint_yn'] = df['Job Description'].apply(lambda x: 1 if 'powerpoint' in x.lower() else 0)
df2.powerpoint_yn.value_counts()

#meta
df2['meta_yn'] = df['Job Description'].apply(lambda x: 1 if 'meta' in x.lower() else 0)
df2.meta_yn.value_counts()
